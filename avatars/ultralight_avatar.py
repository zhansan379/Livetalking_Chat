...
class LightReal(BaseAvatar):
    ...
    def __init__(self, opt, model, avatar):
        ...
        # 注意：ultralight 每形象带一份 ultralight.pth，表情底座需在 data/actions/<e>/
        # 内置自己的 .pth，否则加载失败会优雅回中性（见 base_avatar._load_emotion_cycle）。
        self._snapshot_neutral_cycles()
        def _load_cycle(e, root='data/actions'):
            model_e, fr, fc, co = load_avatar(e, root=root)
    ...
    def paste_back_frame(self,pred_frame,idx:int):
        ...
        crop_img = self.face_list_cycle[idx % max(1, len(self.face_list_cycle))]
