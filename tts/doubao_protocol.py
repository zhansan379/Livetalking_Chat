"""火山引擎豆包双向流式 TTS 的线协议编解码。

直接取自官网示例的 `protocols` 模块（TTS Websocket Bidirection protocols），
仅一处适配：`receive_message` 里的逐帧日志降到 debug，避免音频帧刷屏。
模块内自包含，无额外依赖（websockets 即豆包实现所需）。
"""
